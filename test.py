...
WEBHOOK_HOST = '194.44.128.108'  # Domain name or IP addres which your bot is located.
...
WEBHOOK_SSL_CERT = './server.pem'  # Path to the ssl certificate
...
async def cmd_about(message: types.Message):
    ...
    return SendMessage(message.chat.id, text(
        ...
        '',
        text('I\'m powered by', bold('Python', sys.version_info)),
        text('With', link(text('aiogram', aiogram.__version__), 'https://github.com/aiogram/aiogram')),
        sep='\n'
...
async def unknown(message: types.Message):
    ...
    return SendMessage(message.chat.id,
                       f"I dont know what to do with content type {message.content_type}. Sorry :c")
...
async def on_startup(app):
    ...
    dp.register_message_handler(unknown,
                                lambda message: message.chat.type == ChatType.PRIVATE, content_types=BAD_CONTENT)
